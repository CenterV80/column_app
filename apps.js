...
const APPS = [
  {
    ...
    description: "cfgやstepsを手で試し続けるのをやめ、生成はスクリプト・評価は人間・次の候補決めはClaude Codeに分担させる仕組み。全体フロー、リポジトリ構成、そのまま貼れる依頼文とCLAUDE.mdテンプレートを、用語解説つきでまとめました。",
  ...
  {
    name: "セッションログ運用ワークフロー：AGENTS.md対応版",
    description: "ルール定義を.github/copilot-instructions.mdからAGENTS.mdに移し、Copilot以外のAI（Claude Code、Codexなど）でも同じルールでログを書けるようにする方法をまとめました。",
    path: "apps/session-log-workflow-agents-md.html",
    icon: "🔗",
    category: "ai-coding",
  },
  {
  ...
  },
  {
    ...
    description: "パスワードが必要なページです。絵コンテのコマから MiniMax H3 で動画を生成するための技術仕様書(v0.2ドラフト)を、そのままコピペできるMarkdownで置いています。Ref2V参照・AddGuide・Fun ControlNetの3系統と、2パスサンプリングによる追従強度の制御をまとめました。",
