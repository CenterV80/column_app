...
中間サーバーを省略するため、ワークフローJSONのテンプレートやノード対応表はフロントエンドの静的リソース(またはビルド時埋め込み)として保持する。限られた利用者かつ閉じたネットワーク内であることを前提とし、認証・CORS設定でアクセス範囲を制御する(詳細は8章・9章参照)。
...
- **アクセス制御**: ComfyUIサーバー自体には認証機構が標準搭載されていないため、閉じたネットワーク限定のアクセス(VPN/ローカルLAN)やリバースプロキシでのBasic認証・SSO連携など、ネットワークレイヤーでのアクセス制御が別途必須
