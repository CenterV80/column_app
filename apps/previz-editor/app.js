...
(function () {
  ...
  const state = {
    ...
    objects: [],
    // カットの一覧。中身はシリアライズした形で持ち、切り替えるときに組み立て直す。
    // カットごと: 長さ・配置・アニメ・注視先 ／ 全体共通: fps・補間・進む向き
    shots: [],
    shotIndex: 0,
    selectedId: null,
    nextId: 1,
  ...
  // カットが1つでも出しておく（増やす入口が必要なので）
  function renderShotBar() {
    const bar = $("shotBar");
    let html = "";
    state.shots.forEach((sh, i) => {
      const on = i === state.shotIndex;
      html +=
        '<button class="shot-chip' + (on ? " is-active" : "") + '" data-shot="' + i + '">' +
        escapeHtml(sh.name) +
        (on && state.shots.length > 1 ? '<span class="shot-x" data-del="' + i + '">×</span>' : "") +
        "</button>";
    });
    html += '<button class="shot-add" id="shotAdd" title="いまのカットを複製してふやす">＋</button>';
    bar.innerHTML = html;
  }
  ...
  // いま画面に出ているカットを、保存できる形に落とす
  function serializeShot() {
    const cur = state.shots[state.shotIndex];
    return {
      name: cur ? cur.name : "カット1",
  ...
  function loadShot(shot) {
    ...
    state.duration = Math.max(2, +shot.duration || 120);
    ...
    if (typeof shot.lookAt === "number" && state.objects[shot.lookAt]) {
      const c = theCamera();
      if (c) c.lookAt = state.objects[shot.lookAt].id;
    }
    const cam = theCamera();
  ...
  // カットを切り替える。いまの内容を控えてから、次のカットを開く。
  function switchShot(i) {
    if (i === state.shotIndex || !state.shots[i]) return;
    state.shots[state.shotIndex] = serializeShot();
    state.shotIndex = i;
    loadShot(state.shots[i]);
    renderShotBar();
    syncSettings();
    markDirty();
  }

  // いまのカットを複製して足す。セットとキャラを引き継げるので、動きだけ変えられる。
  function addShot() {
    state.shots[state.shotIndex] = serializeShot();
    const copy = JSON.parse(JSON.stringify(state.shots[state.shotIndex]));
    copy.name = "カット" + (state.shots.length + 1);
    state.shots.splice(state.shotIndex + 1, 0, copy);
    state.shotIndex += 1;
    loadShot(copy);
    renderShotBar();
    markDirty();
    showToast(copy.name + "をふやしました（いまのカットの複製）");
  }
  ...
  function removeShot(i) {
    ...
    state.shots[state.shotIndex] = serializeShot();
    const gone = state.shots[i];
    state.shots.splice(i, 1);
    const at = Math.min(i, state.shots.length - 1);
    state.shotIndex = at;
    loadShot(state.shots[at]);
    renderShotBar();
    syncSettings();
    markDirty();
    showToast(gone.name + "をけしました", () => {
      state.shots[state.shotIndex] = serializeShot();
      state.shots.splice(i, 0, gone);
      state.shotIndex = i;
      loadShot(gone);
      renderShotBar();
      syncSettings();
  ...
  function serialize() {
    ...
    shots[state.shotIndex] = serializeShot(); // 編集中のぶんを反映してから保存する
    return {
      version: 2,
      fps: state.fps,
      ease: state.ease,
  ...
  function loadScene(data) {
    ...
    // 旧い形（カットが1つしかなかった頃）は、1カットとして読み込む
    let shots = Array.isArray(data.shots) ? data.shots : null;
    if (!shots) {
      if (!Array.isArray(data.objects)) return false;
      shots = [{ name: "カット1", duration: data.duration, lookAt: data.lookAt, objects: data.objects }];
    }
    ...
    state.shots = shots.map((sh, i) => ({
      name: sh.name || "カット" + (i + 1),
      duration: Math.max(2, +sh.duration || 120),
      lookAt: typeof sh.lookAt === "number" ? sh.lookAt : null,
      objects: Array.isArray(sh.objects) ? sh.objects : [],
    }));
    state.shotIndex = Math.min(Math.max(0, +data.shotIndex || 0), state.shots.length - 1);

    loadShot(state.shots[state.shotIndex]);
    renderShotBar();
    syncSettings();
    markDirty();
    return true;
  ...
  function depthRange() {
    const camObj = theCamera();
    if (!camObj) return { near: 1, far: 40 };
    let lo = Infinity;
    let hi = 0;
  ...
  async function exportVideo(onProgress, opts) {
    ...
    // どのカットを書き出すか。つなげる場合はいまの内容を控えてから回す。
    const all = !!(opts && opts.allShots) && state.shots.length > 1;
    if (all) state.shots[state.shotIndex] = serializeShot();
    const plan = all ? state.shots.map((_, i) => i) : [state.shotIndex];
    // 継ぎ目でコマが重ならないよう、最後のカット以外は末尾のコマを出さない
    const totalFrames = all
      ? state.shots.reduce((a, sh) => a + Math.max(2, sh.duration), 0) + 1
      : state.duration + 1;

    const keepFrame = state.current;
    const keepShot = state.shotIndex;
    const w = host.clientWidth;
    const h = host.clientHeight;
    ...
    let done = 0;
    try {
      for (const si of plan) {
        if (all && si !== state.shotIndex) {
          state.shotIndex = si;
          loadShot(state.shots[si]);
        }
        const cam = theCamera();
        // 濃淡の範囲はカットごとに測る（同じカットの中ではちらつかない）
        if (depth) {
        ...
        const last = si === plan[plan.length - 1];
        const end = last ? state.duration : state.duration - 1;
        for (let f = 0; f <= end; f++) {
          if (failure) throw failure;
          applyFrame(f);
          // 補助の表示は映さない
          cam.root.userData.rig.visible = false;
          trailGroup.visible = false;
          selRing.visible = false;
          reticle.visible = false;
          grid.visible = !depth; // マス目は距離の絵に混ざるので出さない
          renderer.setViewport(0, 0, VIDEO_W, VIDEO_H);
          renderer.render(scene, cam.root);

          const frame = new VideoFrame(renderer.domElement, {
            timestamp: Math.round((done * 1e6) / state.fps),
            duration: Math.round(1e6 / state.fps),
          });
          // カットの頭は必ずキーフレームにする
          encoder.encode(frame, { keyFrame: f === 0 || done % state.fps === 0 });
          frame.close();

          done++;
          if (onProgress) onProgress(Math.min(done, totalFrames), totalFrames);
          // 画面が固まらないよう、ときどき処理を譲る
          if (encoder.encodeQueueSize > 8 || done % 4 === 0) await new Promise((r) => setTimeout(r, 0));
        }
      }
      await encoder.flush();
      if (failure) throw failure;
      ...
      grid.visible = state.grid;
      renderer.setSize(w, h, false);
      if (all && state.shotIndex !== keepShot) {
        state.shotIndex = keepShot;
        loadShot(state.shots[keepShot]);
        renderShotBar();
      }
      const back = theCamera();
      if (back) back.root.userData.rig.visible = rigWas;
      setFrame(Math.min(keepFrame, state.duration));
      layout();
    }
  ...
  function openExport() {
    ...
    openSheet(
      ...
      '<button class="row-btn" id="bCopyShot">' + icon("i-copy") +
        ...
        (state.shots.length > 1
          ? '<label class="pop-row" style="min-height:38px;border:0;margin-bottom:4px">' +
            '<span class="pop-label">' + state.shots.length + "つのカットをつなげて1本にする</span>" +
            '<span class="sw"><input type="checkbox" id="bJoin" checked><i></i></span></label>'
          : "") +
    ...
    pickVideoFormat().then((fmt) => {
      ...
      const joined = $("bJoin") && $("bJoin").checked;
      const frames = joined
        ? state.shots.reduce((a, sh) => a + Math.max(2, sh.duration), 0) + 1
        : state.duration + 1;
      vSub.textContent =
        fmt.label + " / " + VIDEO_W + "×" + VIDEO_H + " / " + state.fps + "fps / " +
        (frames / state.fps).toFixed(1) + "秒" + (fmt.ext === "mp4" ? "" : "（MP4非対応のブラウザです）");
    });
    ...
    const runExport = (btn, subEl, depth, filename) => async () => {
      ...
      try {
        const out = await exportVideo(
          ...
          { depth: depth, allShots: !!($("bJoin") && $("bJoin").checked) }
        );
  ...
  function setDuration(frames) {
    ...
    if (state.shots[state.shotIndex]) state.shots[state.shotIndex].duration = state.duration;
    state.objects.forEach((o) => {
  ...
  function defaultScene() {
    state.shots = [{ name: "カット1", duration: state.duration, lookAt: null, objects: [] }];
    state.shotIndex = 0;
  ...
  function bind() {
    ...
    $("shotBar").addEventListener("click", (ev) => {
      const del = ev.target.closest(".shot-x");
      if (del) {
        removeShot(+del.dataset.del);
        return;
      }
      if (ev.target.closest("#shotAdd")) {
        addShot();
        return;
      }
      const chip = ev.target.closest(".shot-chip");
      if (chip) switchShot(+chip.dataset.shot);
    });
