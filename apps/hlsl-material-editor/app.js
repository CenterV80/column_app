...
class HlslEditorApp {
  ...
  initPresets() {
    ...
    heading.className = "presets-label";
    heading.textContent = "プリセット";
    box.appendChild(heading);

    for (const [label, code] of SHADER_PRESETS) {
      const btn = document.createElement("button");
      btn.className = "chip";
      btn.type = "button";
      btn.textContent = label;
      btn.addEventListener("click", () => this.applyPreset(code));
      box.appendChild(btn);
    }

    this.el.snippets.parentNode.insertBefore(box, this.el.snippets.nextSibling);
  }

  // プリセットは編集中のコードを丸ごと捨てるので、自分で書いたものが
  // 残っているときだけ確認する。
  applyPreset(code) {
    const current = this.el.code.value;
    const disposable =
      current === this.lastPreset || current.trim() === DEFAULT_CODE.trim();
    if (!disposable && !confirm("編集中のコードを破棄してプリセットを読み込みますか？")) {
      return;
    }
    this.el.code.value = code;
    this.lastPreset = code;
    this.compile();
  }
  ...
  loadState() {
    ...
    // 前回プリセットのまま終えていたなら、次に押すとき確認を挟まない。
    const match = SHADER_PRESETS.find(([, code]) => code === this.el.code.value);
    if (match) this.lastPreset = match[1];
