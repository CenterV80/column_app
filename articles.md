...
[GitHub - gary149/h3-game-sprites](https://github.com/gary149/h3-game-sprites)

動画生成AIで作った映像を、レトロな2Dゲーム用のスプライトシートに変換するエージェントスキル。1992年の格闘ゲーム「モータルコンバット」が実写俳優をデジタル化してキャラクターを作っていた手法を、動画生成モデル（H3など）で再現するというコンセプト。処理パイプラインは、動画からのフレーム抽出とクロマキー合成、キーフレームの自動選択、スプライトアトラス（複数フレームを1枚にまとめた画像）の生成という流れ。技術的な工夫として、キャラクターが必ず元の基準姿勢に戻るようにする「idle pin」機能、足の位置がフレームごとにずれていく「フットドリフト」を防ぐ処理、動きの弧の長さに基づいてキーフレームを選ぶ仕組み、歩行アニメーションが違和感なくループする「シームレスなウォークサイクル」の検出などを備える。必要環境はffmpegとPython 3（Pillow・numpy）に加えて、動画生成モデルと画像生成モデルそれぞれのAPIやローカル実行環境。付属スクリプトは、キャラクターの静止画を生成する`gen_still.py`、動画からスプライトを切り出す`sprite_cut.py`、フレームをアトラス化する`build_atlas.py`の3本立てで、8種類のアニメーションを揃えても約5ドル程度のコストで済むという。
...
[Hugging Face - Comfy-Org/MiniMax-H3 (model_patches)](https://huggingface.co/Comfy-Org/MiniMax-H3/tree/main/model_patches)

Alibaba PAIが公開した、MiniMax-H3向けのControlNet-Union「MiniMax-H3-Fun-Controlnet-Union」を、ComfyUIのネイティブノードでそのまま使えるよう再パッケージ・量子化したパッチファイル（`minimax_h3_fun_controlnet_union_pruned_int8_convrot.safetensors`）。約6.8GBの単一チェックポイントだけで、Canny・Depth・HED・MLSD・Poseという5種類の制御用動画による条件付けと、マスクを使った動画インペイントの両方に対応しており、条件の種類ごとにチェックポイントを切り替える必要がないのが特徴。中身は制御用のブランチのみを持つ構成で、制御用の射影層と5つの制御ブロックを、手元にある通常のMiniMax-H3トランスフォーマーの上に重ねて読み込む形で動作し、fl2va・ref2va両方のトランスフォーマーファイルで利用できる。ComfyUI側もチェックポイントの読み込みから、動画・インペイント条件付け、制御ストリームの実行、ノード統合まで一通りサポートしており、利用にはComfyUI 0.35.0以降が必要。

### MiniMax H3 Camera Control - カメラワークをドラッグ操作でプロンプト化するビジュアルエディタ

*公開: 2026-09-13*

[Civitai - Minimax H3 Camera Control](https://civitai.com/models/2928691/minimax-h3-camera-control)

MiniMax-H3向けにComfyUI内で使える、カメラワークをビジュアルに編集できるツール。名前から連想される、以前紹介したCrossView-WarpのIC-LoRAのように潜在表現を直接操作する幾何学的なカメラアダプターとは異なり、**カメラの動きをドラッグ操作で組み立て、それを説明するプロンプト文をノードが自動生成する**という仕組みである点に注意が必要。そのため、生成されたプロンプトが実際の生成結果に反映されるかはH3側の解釈次第で、角度・タイミング・スケール感が意図通りにならないこともあるとされている。操作方法は、プレビュー上の紫色のカメラアイコンをドラッグして軌道を作り、マウスホイールで距離を変更、背景をドラッグするとプレビューの見え方だけを回転できるといった直感的なUIになっている。オービット（周回）・上昇・下降・接近・離脱・静止といったプリセットも用意されており、キーフレーム間の動きをなめらかに補間する「スムーズモーション」、区間ごとの直線的な動きを保つ「リニアモーション」、キーフレーム間の平均速度をそろえる「タイミング再配分」といったオプションを選べる。
...
fal.ai独自の派生モデル「H3 Max」に搭載された、自己回帰（オートリグレッシブ）方式の生成モード。ベースとなる「H3 Max」自体は、fal Researchがオープンウェイトの MiniMax-H3 をポストトレーニングし、fal の推論チームが速度重視でチューニングしたモデルで、5秒の動画を3秒未満で生成でき、公式のMiniMax-H3エンドポイント比で約35倍のスループットを実現しているという。fal独自の品質評価（人間による選好評価）では、総合品質・プロンプト理解・美観のいずれでも主要な動画生成モデル中トップにランクされているとのこと。今回紹介する「**Director**」モードは、このH3 Maxに最長2分間の文脈（コンテキスト）継続機能を追加した自己回帰版で、5秒や10秒単位で毎回ゼロから生成し直すのではなく、キャラクター・場所・シーンの詳細を連続する生成をまたいで維持できるのが特徴。実際に、観客からのプロンプトを受けてリアルタイムに展開する「終わらないサインフェルド」風のライブ配信で、盗まれたスープのレシピ本を巡る話が続く間もジェリーやクレイマー、ダイナーのセットが一貫して認識できる状態を保てたと報告されている。このDirector層はfal独自のエンジニアリングであり、MiniMax本家のオープンウェイト公開には含まれない、fal専有の機能として位置付けられている。対応解像度は480p・768pで、H3 Max本体が対応する2K出力はDirectorにはまだ来ていない。fal経由では1日5回まで15秒・768p生成を無料で試せる枠が用意されている。
...
MiniMax-H3を使った動画生成を、既存の短いクリップから新しいフレームを生成する形で延長できるようにするComfyUIカスタムノード。前のクリップが持つ運動情報を引き継いで新しいフレームを生成するため、継ぎ目のスムーズな長尺動画を作りやすい。音声面では、前のクリップの音声とタイムラインを同期させたまま続きを生成でき、自動的なオーディオリサンプリングも備えているため、延長のたびに音声がズレていく問題を防止する。画質面でも、光流（オプティカルフロー）ベースの処理でフレーム間のちらつきを抑え、色補正機能で継ぎ目の明るさの変化を自然になじませる工夫がある。動作モードは、潜在表現のまま処理を継続する「潜在空間継続」と、外部の動画ファイルを読み込んで延長する「外部ビデオファイル」の2種類のワークフロー例が付属。インストールはComfyUI Manager経由のワンクリック、Comfy CLI、手動でのGitクローンの3通りに対応する。GPLv3ライセンスで公開。
...
単一のNVIDIA SM89世代GPU（RTX 40シリーズ相当のAdaアーキテクチャ）向けに最適化された、MiniMax-H3をローカルで動かすためのサービス。Webコンソールと REST API を備えた、ComfyUIとは独立したスタンドアロン型のアプリケーションである点が特徴。生成モードはFL2VA（テキストのみ・最初フレーム・最後フレーム・最初＋最後フレームの指定に対応）とRef2VA（画像・映像・独立した音声を含む複数参照からの生成）の両方をサポートし、解像度はネイティブで360p〜1080p、H3のセカンドサンプリングを使えば最大1440pまで対応する。運用面では、BaseモデルとLoRAをその場で切り替えられるホットスイッチング、中断しても再開できるチェックポイント機能、GPUのキュー管理・タスク履歴・ハードウェアテレメトリの確認、そして6種類の独立したランチャー（3つのリソースプロファイルに対応）を備える。VRAMに応じたリソースプロファイルは3段階用意されており、24GB・INT8構成なら1080p×15秒、16GB・INT8構成なら実験的に1080p、8GB・W4A8構成なら720p×15秒での生成に対応する。導入はリポジトリをクローンして`setup.sh`・`run.sh`を実行するだけで、`http://127.0.0.1:8090`のコンソールからすぐに使い始められる。ComfyUIとの連携オプションも用意されている。

### Archify - コードベースをAIコーディングエージェントがそのままシステム図に変換するツール

*公開: 2026-08-27*

[GitHub - tt-a1i/archify](https://github.com/tt-a1i/archify)

コードベースやシステムの説明を、インタラクティブなシステムマップに変換するNode.js製ツール。アーキテクチャ図（コンポーネントやサービス構成）、ワークフロー図（CI/CDや承認フロー）、シーケンス図（API呼び出しや認証）、データフロー図（パイプラインやデータの流れ）、ライフサイクル図（状態遷移やリトライ）という5種類の図表に対応する。特徴的なのは、CursorやClaude Code、Codex CLIといったAIコーディングエージェント環境から「このリポジトリの実行時アーキテクチャをarchifyでマッピングして」のように指示するだけで動く点で、エージェント側が型付きのJSONを生成し、Archifyがそれを決定論的にHTML/SVGへ変換する仕組みになっている。生成された図はノード検索・上流下流の追跡・ルート検証などのインタラクションがすべて元のコードに基づいて動作し、ダーク/ライトテーマや自社ブランドマークの埋め込みにも対応した「そのままプレゼンできる」出力を目指している。PNG・SVG・WebM形式へのエクスポートに加え、マージ前のアーキテクチャ変更をBefore/Delta/Afterの3段階で比較する機能も備える。
