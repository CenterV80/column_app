...
### AI画像・動画生成を支えるPythonライブラリまとめ - 役割ごとに整理
(サイト運営者によるまとめ)

ComfyUIやLTX、FLUX系のソースコードを読んでいると登場する主要ライブラリを役割ごとに整理する。基盤・実行環境としては、**PyTorch**がテンソル計算とニューラルネット実行を担う基盤ライブラリで、GPU上での行列演算を行う「エンジン」部分にあたり、ComfyUIも中身はPyTorchで動いている。**NumPy**はCPU上での配列・数値計算ライブラリで、PyTorch登場以前からある汎用ツールとして画像の前処理（リサイズ、正規化など）でよく使われるが、GPUには対応していない。

モデルの実装・読み込み系では、Hugging Faceの**Transformers**が主にテキストエンコーダー（CLIP、T5など、言葉をベクトルに変換するモデル）の実装を提供し、同じくHugging Faceの**Diffusers**が拡散モデル（Stable Diffusion、FLUXなど）の実装・パイプラインを提供して画像・動画生成本体を担当する。**Safetensors**はモデルの重み（パラメータ）を安全かつ高速に保存・読み込みするためのファイル形式・ライブラリで、計算そのものには関与せず、あくまで重みの「入れ物」にあたる。なお紛らわしいことに、単数形の「Transformer」は2017年の論文"Attention is All You Need"由来のニューラルネット構造そのものを指し、CLIPやT5、拡散モデル内のU-NetやDiTもこの構造を使っている一方、複数形の「Transformers」はこのアーキテクチャを実装したHugging Faceのライブラリの製品名であり、両者は別物である点に注意が必要。

GPU処理まわりでは、NVIDIAの**CUDA**がGPUに直接計算をさせるための基盤の仕組みで、PyTorchなどほとんどのAI系ライブラリはこの上に乗って動いている。**cuDNN**はCUDAの上でニューラルネットが頻繁に使う計算だけを専用に高速化する部品で、CUDAという土台の道路の上を走る専用の高速レーンに例えられる。Meta製の**xFormers**は注意機構（アテンション）の計算を高速化しメモリ消費を抑える専門パーツで、ComfyUIの起動オプションでも名前が登場し、あるかないかで生成速度や使えるメモリ量が変わる。**FlashAttention**は注意機構の計算をメモリの読み書きの順番を工夫することで高速・省メモリに実行できるようにした実装方法で、計算結果自体は変わらず同じ処理をより効率的に行うための工夫であり、xFormersと似た目的を持つ。**Triton**はGPU用の計算コードをPython風の書き方で書けるようにする言語兼コンパイラで、最近のPyTorchの高速化機能の裏側でよく使われている。**bitsandbytes**はモデルの数値を通常より少ないビット数に圧縮し、少ないメモリでも大きなモデルを動かせるようにするライブラリで、FLUXのような大きいモデルを少ないVRAMで動かす量子化処理でよく使われる。Hugging Faceの**Accelerate**は複数GPUやさまざまな環境でコードを書き換えずに動かせるようにする実行環境の橋渡し役で、1枚のGPUだけで動かしている分にはあまり意識しなくてよい。

周辺ツールとしては、**Einops**が配列やテンソルの形を次元の名前を書くだけで自由に並び替え・整理できるライブラリで、画像データの形状変換（縦横チャンネル数の順番からバッチ・チャンネル・縦横の順番への入れ替えなど）を短いコードで書けるようにし、地味だがソースコード中に頻出する。**Gradio**はPythonコードにボタン・スライダー・画像アップロード欄などの操作画面を数行で追加できるライブラリで、Hugging Face Spacesの多くはこれで作られている。**OpenCV**は画像・動画処理の定番ライブラリで、リサイズ・色変換・輪郭検出など画像処理全般をカバーし、ControlNet用の線画・深度マップ作成の前処理でもよく使われる。全体の処理の流れをイメージすると、入力データをNumPyやOpenCVで前処理してテンソル化し、PyTorch（CUDA・cuDNN・Triton・xFormers・FlashAttentionで高速化）上でTransformersがプロンプトをベクトルに変換、それをDiffusersのUNetやDiTなどのパイプラインが受け取って画像・動画を生成し、最後にSafetensors形式で重みを読み書きする、という流れになる。
